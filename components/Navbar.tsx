...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-studio-black/90 backdrop-blur-md border-b border-white/10" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <span className="text-white font-bold text-lg tracking-tight">
          Shergutt<span className="text-accent">.</span>Studio
        </span>

        <div className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-white/60 hover:text-accent text-sm transition-colors duration-200">
              {l.label}
            </a>
          ))}
        </div>
      ...
      {open && (
        <div className="md:hidden bg-studio-dark border-b border-white/10 px-6 py-6 flex flex-col gap-4">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-white/70 hover:text-accent text-base transition-colors" onClick={() => setOpen(false)}>
              {l.label}
            </a>
          ))}
